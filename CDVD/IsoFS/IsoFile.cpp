...
{
        ASSERT( dest != NULL );
        ASSERT( len >= 0 );           // should we silent-fail on negative length reads?  prolly not...
		...
		s32 minLen = sectorLength - sectorOffset;
		if (len < minLen)
			minLen = len;
//        int firstSector = internalRead(dest, off, std::min(len, sectorLength - sectorOffset));
        int firstSector = internalRead(dest, off, minLen);
        off += firstSector;
        len -= firstSector;
...
CString IsoFile::readLine()
{
        //std::string s;
        //s.reserve( 512 );
        CString str;
        PTSTR ps = str.GetBuffer( 512 );
