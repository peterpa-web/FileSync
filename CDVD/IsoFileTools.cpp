...
//      Console.WriteLn("_openfile %s, %d", filename, flags & O_RDONLY);
        if (flags & O_WRONLY)
        {
                ...
                handle = CreateFileA(fromUTF8(filename), GENERIC_WRITE, 0, NULL, _flags, 0, NULL);
        }
        else
        {
                handle = CreateFileA(fromUTF8(filename), GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
        }
